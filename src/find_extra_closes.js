...
for (let L = 0; L < lines.length; L++) {
    ...
    for (let i = 0; i < line.length; i++) {
        ...
        if (inString) { 
            if (char === inString) { 
                let bs = 0; for (let j = i - 1; j >= 0 && line[j] === '\\'; j--) bs++; 
    ...
    if (braces < 0) { console.log(`CRITICAL: EXTRA } at line ${L+1}: ${line.trim()}`); braces = 0; }
    if (parens < 0) { console.log(`CRITICAL: EXTRA ) at line ${L+1}: ${line.trim()}`); parens = 0; }
    if (brackets < 0) { console.log(`CRITICAL: EXTRA ] at line ${L+1}: ${line.trim()}`); brackets = 0; }
}
console.log(`Final Balances: B=${braces}, P=${parens}, Br=${brackets}`);
